...
// 加载所有景点图片(图片由后端代理,无需前端fetch,直接将img指向后端接口)

// 获取景点图片: 直接指向后端代理接口,由后端转发Unsplash图片字节流,
// 避免浏览器直连images.unsplash.com被网络环境拦截导致图片不显示
const getAttractionImage = (name: string): string => {
  return `http://localhost:8000/api/poi/photo?name=${encodeURIComponent(name)}`
}
...
const exportAsImage = async () => {
  try {
    ...
    cardHeads.forEach((head) => {
      ...
      try {
        headEl.style.setProperty('background-color', '#1E5EFF')
    ...
    hotelCards.forEach((card) => {
      ...
      if (head) {
        head.style.setProperty('background-color', '#2B6DE8')
      }
      (card as HTMLElement).style.setProperty('background-color', '#EDF3FF')
    })
    ...
    if (budgetTotal) {
      ...
      el.style.setProperty('background-color', '#1E5EFF')
...
const addAttractionMarkers = (AMap: any) => {
  ...
  allAttractions.forEach((attraction, index) => {
    const marker = new AMap.Marker({
      ...
      label: {
        content: `<div style="background: #1E5EFF; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px;">${index + 1}</div>`,
    ...
    const infoWindow = new AMap.InfoWindow({
      content: `
        <div style="padding: 10px;">
          ...
          <p style="margin: 4px 0; color: #1E5EFF;"><strong>第${attraction.dayIndex + 1}天 景点${attraction.attrIndex + 1}</strong></p>
        </div>
      `,
      offset: new AMap.Pixel(0, -30)
    })
...
/* 天气卡片样式 */
.weather-card {
  background: linear-gradient(135deg, #EDF3FF 0%, #DCE9FF 100%);
...
.weather-wind {
  margin-top: 8px;
  padding-top: 8px;
  border-top: 1px solid rgba(43, 109, 232, 0.2);
  text-align: center;
  color: #2B6DE8;
  font-size: 14px;
}
...
.hotel-card :deep(.ant-card-head) {
  background: linear-gradient(135deg, #2B6DE8 0%, #1E5EFF 100%);
}
