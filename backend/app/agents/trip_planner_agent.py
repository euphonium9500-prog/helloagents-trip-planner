...
class FixedSimpleAgent(SimpleAgent):
    """修复hello_agents SimpleAgent的兼容性问题:

    原实现在工具调用后,将移除[TOOL_CALL]标记的响应作为assistant消息发回。
    当LLM的响应只包含工具调用标记时,移除后为空字符串,
    Moonshot等API会拒绝空的assistant消息(400错误)。
    """
    ...
    def run(self, input_text: str, max_tool_iterations: int = 3, **kwargs) -> str:
        ...
        while current_iteration < max_tool_iterations:
            ...
            if tool_calls:
                ...
                # 修复: 空的assistant消息会被API拒绝,填入占位内容
                clean_response = clean_response.strip()
                if not clean_response:
                    clean_response = "(已调用工具,正在等待工具结果)"
...
class MultiAgentTripPlanner:
    ...
    def __init__(self):
        ...
        try:
            ...
            # 创建景点搜索Agent
            print("  - 创建景点搜索Agent...")
            self.attraction_agent = FixedSimpleAgent(
                name="景点搜索专家",
                llm=self.llm,
            ...
            # 创建天气查询Agent
            print("  - 创建天气查询Agent...")
            self.weather_agent = FixedSimpleAgent(
                name="天气查询专家",
                llm=self.llm,
            ...
            # 创建酒店推荐Agent
            print("  - 创建酒店推荐Agent...")
            self.hotel_agent = FixedSimpleAgent(
                name="酒店推荐专家",
                llm=self.llm,
